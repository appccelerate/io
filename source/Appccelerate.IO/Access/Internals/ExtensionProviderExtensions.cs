...
namespace Appccelerate.IO.Access.Internals
{
    ...
    public static class ExtensionProviderExtensions
    {
        ...
        private static Item CacheItem<TExtension>(string methodName, Key key, bool hasReturn, Type returnType)
        {
            ...
            IEnumerable<MethodInfo> candidates = GetMethodCandidates<TExtension>(beginMethodName, endMethodName, failMethodName).ToArray();
                         ...
                         where TExtension : class
        {
            ...
            {
                Exception[] parameters = !exception.Equals(args.Single()) ? new[] { exception } : args;
                // ReSharper disable once CoVariantArrayConversion
                item.Fail.Invoke(extension, parameters);
            }
